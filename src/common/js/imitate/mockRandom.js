 ...
 */
const postData = () => {
  let dataList = []; // 用于存放文章数据的数组
  for (let i = 0; i < 10; i++) {
    let post = {
      title: Random.ctitle(10, 25), // 随机生成长度为10-25的标题
...
const sellerData = () => {
  ...
  for (let i = 0; i < 1; i++) {
    dataList.push({
      "name": Random.ctitle(10, 25),
      "description": Random.cparagraph(1, 1),
      "deliveryTime": Random.date("mm"),
      "score": Random.float(0, 5, 0, 1),
      "serviceScore": Random.float(0, 10, 0, 1),
      "foodScore": Random.float(0, 10, 0, 1),
      "rankRate": Random.float(0, 100, 0, 1),
      "minPrice": Random.integer(0, 10000),
      "deliveryPrice": Random.integer(0, 10000),
      ...
      "avatar": Random.dataImage("64x64", "hello"),
      "pics": [
        Random.dataImage("64x64", "hello"),
        Random.dataImage("64x64", "hello"),
        Random.dataImage("64x64", "hello"),
        Random.dataImage("64x64", "hello")
      ],
      "infos": [
        Random.cparagraph(2, 6),
        Random.cparagraph(12, 16),
        Random.cparagraph(6, 22),
        Random.cparagraph(12, 26)
      ]
    });
  };
  return {
